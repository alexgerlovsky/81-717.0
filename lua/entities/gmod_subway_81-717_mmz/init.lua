...
ENT.SyncTable = {
    ...
    "CustomE","CustomF","CustomG","R_UNch","R_ZS","R_G","R_Radio","R_Program1","R_Program2","R_Program1H","R_Program2H","KB1","KB2","KVT","EPKb","ContVent",
    "VMK","VUD","KDL","KDLR","KDLK","KDP","KDLRK",
    "KRZD","VozvratRP","KVP","ConverterProtection","RZP","RZPud","RZPpod","GreenRP",
    "KSN","ARS","ARSR","ALS","OVT","L_2","L_3","DIPoff",
    "VMK","BPSNon","VAO","RezMK","ARS13","Radio13","L_4","VUS","VAH","VAD","KRP",
    "KDPK",
    "A53","A56","A54","A17","A44","A39","A70","A14","A26","AR63","AS1","A13","A21","A31","A32","A16","A12","A24","A49","A27","A72","A50","A29","A46","A47","A71","A7","A9","A8","A52","A19","A48","A10","A22","A30","A1","A2","A3","A4","A5","A6","A20","A25","A11","A37","A45","A18","A51","A65","A42","A43","A41","A40","A60","A58","A55","A57","A59","A61","A28","A66",
    "AV2","AV3","AV4","AV5","AV6","AV1",
    "A15",
    --"AA",
    "RC1","RC2","VB","VA","BPS", "PB", "UAVA",
...
function ENT:Initialize()
    ...
    self.Lights = {
        ...
        -- Cabin
        [10] = { "dynamiclight",        Vector( 425, 0, 30), Angle(0,0,0), Color(255,245,245), --[[Color(216,161,92),]] distance = 550, brightness = 0.25},
...
local function rnd(num,max)
    ...
    self.FrontCouple.CoupleType = "722"
    self.RearCouple.CoupleType = "717" 
...
--[[
    ...
    --if self.CustomSettings then
		...
		self:SetNW2Int("MaskType", masktype)
        ...
        --self:SetNW2Bool("LVZ",typ==2)
           ...
           -- self:SetNW2Bool("Mask22",mask==1)
            ...
            self:SetNW2Int("BPSNType",(math.random()>0.7 and 2 or 1) or 2+math.Clamp(math.floor(math.random()*11)+1,1,2))
    ...
    --[[else
        ...
        self:SetNW2Int("BPSNType",(math.random()>0.7 and 2 or 1) or 2+math.Clamp(math.floor(math.random()*11)+1,1,11))
    ...
    --Блок смены уставок
    self.RP1_3:TriggerInput("TrigLev",math.random(620,660))
    self.RP2_4:TriggerInput("TrigLev",math.random(620,660))
    self.RV3:TriggerInput("OpenTime",2.3+math.random()*0.2)
    self.Rper:TriggerInput("OpenTime",0.7+math.random()*0.2)
...
function ENT:Think()
    ...
    --Бортовые зеленые лампы
    self:SetPackedBool("GRP",self.Panel.GreenRP > 1)
    self:SetPackedBool("GRP_dim",self.Panel.GreenRP == 0.5)
    self:SetPackedBool("BrW",self.Panel.BrW > 0)

    -- Switch and button states
    --Зеленая лампа РП на 4-ом блоке
    ...
    if Panel.LSN > 0 then
        for i,v in ipairs(self.WagonList) do
            TW18 = TW18+v.Panel.TW18
        end
    end
	
	self:SetPackedBool("RP",TW18 > 1)
    self:SetPackedBool("SN",TW18 > 0)
    self:SetPackedRatio("RPR",math.Clamp((TW18/wags)^0.7,0,1))
	...
	self:SetPackedBool("BLD",self.ALS_ARS.F6 > 0)
    ...
    self:SetPackedBool("LKV1",Panel.LKV1 > 0)
    self:SetPackedBool("LKV2",Panel.LKV2 > 0)
	...
	self:SetNW2Int("ALSSpeed", self.ISDPM.Speed)
    ...
    local EKK_VK = self:ElectricConnected(self.RearTrain,true)
    self.Electric:TriggerInput("EKK_VK", EKK_VK and 1 or 0)
