...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_714_MMZ_Electric")
TRAIN_SYSTEM.MVM = 1
...
function TRAIN_SYSTEM:Inputs(...)
    return { "Type", "NoRT2", "HaveVentilation" }
end
...
function TRAIN_SYSTEM:SolveAllInternalCircuits(Train,dT,firstIter)
    ...
    Train.LK2:TriggerInput("Set",S["20B"]*S["ZR"])
    Train.LK5:TriggerInput("Set",S["20B"]*S["ZR"])
    ...
    P:TriggerInput("PP",S["1A"]*C(RK==18)*Train.LK1.Value*S["ZR"])--1A-1D
    ...
    --+B
    S["1N"] = C(11<=RK and RK<=18)*(1-Train.LK5.Value)
    S["RR"] = S["10A"]*(S["1N"] + P.PS*(C(RK==1)*Train.LK5.Value+Train.LK4.Value))
    Train.RR:TriggerInput("Set",S["RR"])
    Train.Rper:TriggerInput("Set",S["RR"])

    S["5Zh"] = S["10A"]*(1-Train.LK3.Value)*(1-Train.LK1.Value)*(1-Train.LK4.Value)
    ...
    S["2Ca"] = P.PS*C(1<=RK and RK<=17)*Train.Rper.Value --CHECK
    S["2Cb"] = P.PP*(C(6<=RK and RK<=18)+C(2<=RK and RK<=5)*Train.KSH1.Value)*(1-Train.Rper.Value) --CHECK
    ...
    S["10BG"] = Train.TR1.Value + Train.RV1.Value
    ...
    S["18A"] = (S["10AN"]*100+(1-Train.LK4.Value))*Train.A18.Value
    ...
    Panel.RZP = T[61]*Train.RZP.Value
    ...
    --Вспом цепи приём
    Panel.EmergencyLights = T[11]*Train.A15.Value
    ...
    Train.RPU:TriggerInput("Set",T[37]*Train.A37.Value)
    Train.RZPud:TriggerInput("Set",T[36]*Train.A38.Value*(1-Train.RPU.Value)*Train.RZP.Value)
    Train.RZP:TriggerInput("Set",Train.RZPpod.Value+Train.RZPud.Value)
    Train:WriteTrainWire(61,T[36])
...
local wires = {10,1,6,3,20,4,5,2,48,8,39,19,25,13,-13,14,24,17,71,11,36,-28,37,16,12,31,32,69,27,23,22,23,37,59,60,58,61}
